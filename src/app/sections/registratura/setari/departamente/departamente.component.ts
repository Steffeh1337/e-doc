...
@Component({
	...
	styleUrls: ['./departamente.component.sass']
})
...
export class DepartamenteComponent implements OnInit {
	...
	editErrorLabel: string = environment.config.customNotifications.registratura.setari.departamente.editError;
	addErrorLabel: string = environment.config.customNotifications.registratura.setari.departamente.addError;
	editSuccessLabel: string = environment.config.customNotifications.registratura.setari.departamente.editSuccess;
	addSuccessLabel: string = environment.config.customNotifications.registratura.setari.departamente.addSuccess;
	...
	constructor(
		...
		private departamenteService: DepartamenteService,
	...
	async editDepartment(id){
		...
		self.departamenteService.findDepartment(id)
		.then(async (res) => {
				...
				if (response.status_code == 200 && typeof response.data !== 'undefined') {
					...
					.subscribe(res => {
						if(res){
							...
							self.departamenteService.updateDepartment(dataToSend)
							.then(async (res) => {
								if(res.errors == false){
									self.loading = false;
									self.rerender();
	...
	async addDepartment(){
		...
		.subscribe(res => {
			if(res){
				...
				self.departamenteService.storeDepartment(dataToSend)
				.then(async (res) => {
					if(res.errors == false){
						self.loading = false;
						self.rerender();
