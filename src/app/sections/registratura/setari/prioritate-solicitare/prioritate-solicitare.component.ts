...
@Component({
	selector: 'prioritate-solicitare',
	templateUrl: './prioritate-solicitare.component.html',
	styleUrls: ['./prioritate-solicitare.component.sass']
})
export class PrioritateSolicitareComponent implements OnInit {
	...
	editErrorLabel: string = environment.config.customNotifications.registratura.setari.prioritatiSolicitari.editError;
	addErrorLabel: string = environment.config.customNotifications.registratura.setari.prioritatiSolicitari.addError;
	editSuccessLabel: string = environment.config.customNotifications.registratura.setari.prioritatiSolicitari.editSuccess;
	addSuccessLabel: string = environment.config.customNotifications.registratura.setari.prioritatiSolicitari.addSuccess;
	...
	constructor(
		...
		private prioritateSolicitareService: PrioritateSolicitareService,
		...
		this.dtOptions = {
			...
			ajax: (dataTablesParameters: any, callback) => {
				...
				.post<DataTablesResponse>(
					'http://ps6-web.back.lc/api/ticketpriorities',
				...
				subscribe(response => {
					...
					$(function(){
						$('.edit-priority').on('click', function(){
							let id_ticket_priority = $(this).val();
							self.editPriority(id_ticket_priority);
						});
					});
			...
			columns: [
				{ title: 'ID', data: 'id_ticket_priority' },
	...
	async addPriority(){
		...
		.subscribe(res => {
			if(res){
				...
				self.prioritateSolicitareService.storeSolicitarePriority(dataToSend)
				.then(async (res) => {
					if(res.errors == false){
						self.loading = false;
						self.rerender();
	...
	async editPriority(id){
		
		var self = this;

		self.prioritateSolicitareService.getSolicitarePriority(id)
		.then(async (res) => {
			...
			if (typeof response.status_code !== 'undefined') {
				if (response.status_code == 200 && typeof response.data !== 'undefined') {
					...
					.subscribe(res => {
						if(res){
							let dataToSend = {
								id_ticket_priority: id,
								name: res.data.name,
								color: res.data.color,
								time: res.data.time
							};
							
							self.prioritateSolicitareService.updateSolicitarePriority(dataToSend)
							.then(async (res) => {
								if(res.errors == false){
									self.loading = false;
									self.rerender();
