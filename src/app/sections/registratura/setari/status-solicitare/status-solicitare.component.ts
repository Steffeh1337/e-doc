...
@Component({
	selector: 'status-solicitare',
	templateUrl: './status-solicitare.component.html',
	styleUrls: ['./status-solicitare.component.sass']
})

export class StatusSolicitareComponent implements OnInit {
	...
	editErrorLabel: string = environment.config.customNotifications.registratura.setari.statusuriSolicitari.editError;
	addErrorLabel: string = environment.config.customNotifications.registratura.setari.statusuriSolicitari.addError;
	editSuccessLabel: string = environment.config.customNotifications.registratura.setari.statusuriSolicitari.editSuccess;
	addSuccessLabel: string = environment.config.customNotifications.registratura.setari.statusuriSolicitari.addSuccess;
	...
	constructor(
		...
		private statusSolicitareService: StatusSolicitareService,
		...
		this.dtOptions = {
			...
			ajax: (dataTablesParameters: any, callback) => {
				...
				.post<DataTablesResponse>(
					'http://ps6-web.back.lc/api/ticketstatuses',
				...
				subscribe(response => {
					...
					$(function(){
						$('.edit-status').on('click', function(){
							let id_ticket_status = $(this).val();
							self.editStatus(id_ticket_status);
						});
					});
			...
			columns: [
				{ title: 'ID', data: 'id_ticket_status' },
	...
	addStatus(){
		...
		.subscribe(res => {
			if(res){
				...
				self.statusSolicitareService.storeSolicitareStatus(dataToSend)
				.then(async (res) => {
					if(res.errors == false){
						self.loading = false;
						self.rerender();
	...
	editStatus(id){
		...
		self.statusSolicitareService.getSolicitareStatus(id)
		.then(async (res) => {
				...
				if (response.status_code == 200 && typeof response.data !== 'undefined') {
					...
					.subscribe(res => {
						if(res){
							...
							self.statusSolicitareService.updateSolicitareStatus(dataToSend)
							.then(async (res) => {
								if(res.errors == false){
									self.loading = false;
									self.rerender();
