...
export class PrioritateSesizareComponent implements OnInit {
	...
	editErrorLabel: string = environment.config.customNotifications.mobile.setari.prioritatiSesizari.editError;
	addErrorLabel: string = environment.config.customNotifications.mobile.setari.prioritatiSesizari.addError;
	editSuccessLabel: string = environment.config.customNotifications.mobile.setari.prioritatiSesizari.editSuccess;
	addSuccessLabel: string = environment.config.customNotifications.mobile.setari.prioritatiSesizari.addSuccess;
	...
	constructor(
		...
		private prioritateSesizareService: PrioritateSesizareService,
				...
				subscribe(response => {
					...
					$(function(){
						$('.edit-priority').on('click', function(){
							let id_sesizare_priority = $(this).val();
							self.editPriority(id_sesizare_priority);
						});
					});
	...
	async addPriority(){
		...
		.subscribe(res => {
			if(res){
				...
				self.prioritateSesizareService.storeSesizarePriority(dataToSend)
				.then(async (res) => {
					if(res.errors == false){
						self.loading = false;
						self.rerender();
	...
	async editPriority(id){
		...
		self.prioritateSesizareService.getSesizarePriority(id)
		.then(async (res) => {
				...
				if (response.status_code == 200 && typeof response.data !== 'undefined') {
					...
					.subscribe(res => {
						if(res){
							...
							self.prioritateSesizareService.updateSesizarePriority(dataToSend)
							.then(async (res) => {
								if(res.errors == false){
									self.loading = false;
									self.rerender();
